...
const PromoCard = (props) => {
    ...
    return (
        <Card
            ...
            />
            <CardContent style={{
                padding: isDialog ? '40px 75px 16px 75px' : openDetail ? '16px 24px 8px 24px' : '8px 16px 16px 16px',
            }}>
            ...
            <Collapse in={openDetail} timeout="auto" unmountOnExit >
                <Box sx={{ padding: isDialog == true ? '0px 75px 5px 75px' : openDetail ? '0px 24px 24px 24px' : '0px 16px 8px 16px', maxHeight: 190, overflowY: 'scroll' }}>
                    <Typography sx={{ color: theme.palette.text.primary, fontSize: 12, fontFamily: 'Eina04-Regular' }}>
                        {promo.detail}
                    </Typography>
                </Box>
                <Box sx={{ padding: isMobile ? openDetail ? '8px 24px 16px 24px' : '8px 16px 16px 16px' : '30px 75px 30px 75px' }}>
                    {isDialog ?
                        (<>
                            <DivFlexCenter sx={{ mb: 2 }}>
                                <Typography fontSize={12} sx={{ fontFamily: 'Eina04-SemiBold', color: theme.palette.text.primary }}>
                                    <span> <PhoneIcon color={theme.palette.text.primary} sx={{ fontSize: 16, mr: 0.5, mt: -0.2 }} /> </span> +62 123 4567 89
                                </Typography>
                            </DivFlexCenter>

                            <Button isPromo={true} icon={<ChatIcon color={'#F4F4F4'} sx={{ mr: 1, fontSize: 20 }} />} theme={theme}>
